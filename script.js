document.addEventListener('DOMContentLoaded', function () {
  ...
  function card(label, title, description, url, playlist) { const a=document.createElement('a'); a.className='card'; a.href=url; a.target='_blank'; a.rel='noopener'; a.innerHTML='<small>'+label+'</small><h3>'+title+'</h3><p>'+description+'</p><b>'+(playlist?'Open playlist':'Watch on YouTube')+' ↗</b>'; return a; }
  const lectureCards=document.getElementById('lecture-cards'), playlistCards=document.getElementById('playlist-cards');
  if(lectureCards) lectures.forEach((x,i)=>lectureCards.appendChild(card(x[0]+' · '+String(i+1).padStart(2,'0'),x[1],x[2],x[3],false)));
  if(playlistCards) playlists.forEach((x,i)=>playlistCards.appendChild(card(x[0]+' · '+String(i+1).padStart(2,'0'),x[1],'A focused Saem Physics learning path.',x[2],true));
  ...
  const items=document.querySelectorAll('.reveal'); if('IntersectionObserver' in window){const io=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('visible');io.unobserve(e.target);}}),{threshold:.1});items.forEach(x=>io.observe(x));}else items.forEach(x=>x.classList.add('visible'));
  const canvas=document.getElementById('space'); if(canvas&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches){const ctx=canvas.getContext('2d');let ps=[];function resize(){canvas.width=innerWidth;canvas.height=innerHeight;ps=Array.from({length:Math.min(70,Math.floor(innerWidth/18))},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.35,vy:(Math.random()-.5)*.35,r:Math.random()*1.5+.4}));}function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ps.forEach((p,i)=>{p.x=(p.x+p.vx+canvas.width)%canvas.width;p.y=(p.y+p.vy+canvas.height)%canvas.height;ctx.fillStyle='rgba(104,217,243,.4)';ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();for(let j=i+1;j<ps.length;j++){const q=ps[j],d=Math.hypot(p.x-q.x,p.y-q.y);if(d<115){ctx.strokeStyle='rgba(104,217,243,'+(.1*(1-d/115))+')';ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.stroke();}}});requestAnimationFrame(draw);}addEventListener('resize',resize);resize();draw();}
});
